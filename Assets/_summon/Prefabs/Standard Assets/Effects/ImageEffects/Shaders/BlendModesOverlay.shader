...
Shader "Hidden/BlendModesOverlay" {
	...
	v2f vert( appdata_img v ) { 
		...
		o.uv[0] = (o.uv[0]-half2(0.5,0.5)) * _Scale+half2(0.5,0.5);
...
if (Target <= ½) R = (2xTarget) x Blend		
	...
	half4 fragAlphaBlend (v2f i) : SV_Target {
		...
		toAdd.a = max((toAdd.r-0.1),0)/(toAdd.r-0.1)*0.5;
		
		return lerp(lerp(lerp(tex2D(_MainTex, i.uv[1]), toAdd, toAdd.a * _Intensity),half4(1,1,1,1),_White),half4(0,0,0,1),saturate(1-_Time.y));
	}	
